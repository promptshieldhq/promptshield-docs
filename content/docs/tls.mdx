...
The included `deploy/Caddyfile` sets `flush_interval -1` so SSE streaming works correctly.

## Option 2: nginx + certbot

```bash
sudo cp deploy/nginx.conf /etc/nginx/sites-available/promptshield
...
The included `deploy/nginx.conf` sets `proxy_buffering off`, which is required for SSE streaming to work.
